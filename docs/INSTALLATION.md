...
```xml
<plugin name="cordova-plugin-push-notifications" spec="1.0.0" />
```
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `cordova-plugin-push-notifications`, for the following error will rise during the build process:
...
```log
Installing "cordova-plugin-push-notifications" for ios
Failed to install 'cordova-plugin-push-notifications':Error: pod: Command failed with exit code 1
